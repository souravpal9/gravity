...
const Login = () => {
    ...
    const handleSubmit = (e) => {
        ...
        setIsLoading(true);
        // Small timeout so UI updates before sync localStorage read
        setTimeout(() => {
            ...
            if (result.user?.mode) setMode(result.user.mode);
        ...
        <div className="flex items-center justify-center min-h-[80vh] px-4">
            <div className={`w-full max-w-md ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-2xl shadow-xl p-8`}>
                ...
                <div className="text-center mb-8">
                    ...
                    <h1 className={`text-2xl font-bold mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>Welcome back</h1>
                    <p className={`text-sm capitalize ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        Sign in to your {cfg.label} account
                    </p>
                </div>
                ...
                <p className={`mt-6 text-center text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                    ...
                    <Link to="/signup" className={`font-semibold hover:underline text-${cfg.accent}-${isDark ? '400' : '600'}`}>
                        Sign up free
                    </Link>
                </p>
            </div>
