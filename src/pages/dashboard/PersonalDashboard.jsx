...
const PersonalDashboard = () => {
    ...
    const chats = [
        { id: 1, name: 'Ava Patel', preview: 'Brunch at 11? I booked a table.' },
        { id: 2, name: 'Jordan Lee', preview: 'Sent you the reel—too funny 😂' },
        { id: 3, name: 'Mia Rodriguez', preview: 'Can you review the photos tonight?' },
        { id: 4, name: 'Noah Kim', preview: 'Running 10 min late—start without me.' },
        { id: 5, name: 'Sofia Chen', preview: 'Let’s hop on a quick call after work.' },
        { id: 6, name: 'Zara Ahmed', preview: 'Loved the playlist you shared!' }
    ];

    const activeChatName = chats.find((chat) => chat.id === activeChat)?.name || 'Conversation';
    const displayMessages = messages.length
        ? messages
        : [
            {
                id: 'seed-1',
                text: 'Hey! Just landed. Want to grab dinner later?',
                time: '6:42 PM',
                user: { id: 'friend-1', name: activeChatName }
            },
            {
                id: 'seed-2',
                text: 'Yes! I can do 8. Any place in mind?',
                time: '6:43 PM',
                user: { id: user?.id || 'me', name: user?.name || 'You' }
            },
            {
                id: 'seed-3',
                text: 'Let’s try the new ramen spot near the station.',
                time: '6:44 PM',
                user: { id: 'friend-1', name: activeChatName }
            }
        ];
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {chats.map((chat) => (
                            <div
                                ...
                                className={`p-4 cursor-pointer transition-colors border-b border-slate-100 dark:border-primary-900/10 last:border-0 ${activeChat === chat.id ? 'bg-primary-50 dark:bg-primary-900/20' : 'hover:bg-primary-50/60 dark:hover:bg-primary-900/10'}`}
                            >
                                <div className="flex gap-3 items-center">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-1">
                                            <h4 className="font-semibold text-slate-800 dark:text-primary-50 truncate">{chat.name}</h4>
                                            <span className="text-xs text-slate-500">12:30 PM</span>
                                        </div>
                                        <p className="text-slate-500 dark:text-slate-400 text-sm truncate">{chat.preview}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                            ...
                            <>
                                ...
                                <div className="p-4 border-b border-slate-200 dark:border-primary-800 flex justify-between items-center bg-white/80 dark:bg-primary-900/80 backdrop-blur-sm flex-shrink-0">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-bold text-slate-800 dark:text-primary-50">{activeChatName}</h3>
